...
namespace UserControl_role_privilege_mapping
  ...
  public partial class TWebUserControl_role_privilege_mapping : ki_web_ui.usercontrol_class
    ...
    private void InitializeComponent()
      ...
      GridView_control.RowDeleting += new GridViewDeleteEventHandler(GridView_control_RowDeleting);
      PreRender += TWebUserControl_role_privilege_mapping_PreRender;
      }

    private void TWebUserControl_role_privilege_mapping_PreRender(object sender, EventArgs e)
      {
    ...
    private void GridView_control_RowDeleting(object sender, GridViewDeleteEventArgs e)
      {
      p.biz.role_privilege_map.Save
        (
        member_id:k.Safe(GridView_control.Rows[e.RowIndex].Cells[Static.CI_PRIVILEGE_ID].Text,k.safe_hint_type.NUM),
    ...
    private void GridView_control_RowDataBound(object sender, GridViewRowEventArgs e)
      ...
      if (e.Row.RowType != DataControlRowType.EmptyDataRow)
        ...
        e.Row.Cells[Static.CI_PRIVILEGE_ID].Visible = false;
        }
      }

    protected void Button_add_Click(object sender, EventArgs e)
      {
      p.biz.role_privilege_map.Save(k.Safe(DropDownList_privilege.SelectedValue, k.safe_hint_type.NUM), k.Safe(DropDownList_role.SelectedValue, k.safe_hint_type.NUM), true);
      Bind();
      }
    ...
    private void Bind()
      {
      p.biz.role_privilege_map.BindActuals(p.sort_order, p.be_sort_order_ascending, GridView_control);
      TableCell_add_mapping.Visible = p.may_add_mappings;
      if (TableCell_add_mapping.Visible)
        {
        p.biz.roles.BindDirectToListControl(DropDownList_role, k.Has((string[])(Session["privilege_array"]), "config-roles-and-matrices"));
        p.biz.privileges.BindDirectToListControl(DropDownList_privilege);
        }
