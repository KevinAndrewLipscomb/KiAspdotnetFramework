  ...
  public partial class TWebUserControl_role_privilege_matrix: ki_web_ui.usercontrol_class
    ...
    private struct p_type
    {
            ...
            if (row.Cells.Count > Static.CI_FIRST_CROSSTAB)
            {
                for (i = Static.CI_FIRST_CROSSTAB; i <= (row.Cells.Count - 1); i ++ )
                {
                    if (row.RowType == DataControlRowType.DataRow)
                    {
                        ...
                        check_box.ID = k.EMPTY + Static.CHECKBOX_ID_PREFIX_PRIVILEGE_ID + row.Cells[Static.CI_PRIVILEGE_ID].Text + Static.CHECKBOX_ID_PREFIX_ROLE_ID + crosstab_metadata_rec.id;
        ...
        private void Changed(object sender, System.EventArgs e)
        {
            ...
            tuple = check_box.ID.Split(new string[] {Static.CHECKBOX_ID_PREFIX_PRIVILEGE_ID, Static.CHECKBOX_ID_PREFIX_ROLE_ID}, StringSplitOptions.RemoveEmptyEntries);
        ...
        private void GridView_control_RowDataBound(object sender, System.Web.UI.WebControls.GridViewRowEventArgs e)
        {
            if (e.Row.RowType != DataControlRowType.EmptyDataRow)
            {
                ...
                e.Row.Cells[Static.CI_PRIVILEGE_NAME].Wrap = false;
                Checkboxify(e.Row);
            }
