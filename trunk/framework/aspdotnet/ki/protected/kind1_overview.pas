...
type
  p_type =
    ...
    tcci_kind3_name: cardinal;
    tcci_kind3_dictated_appropriation_id: cardinal;
    tcci_kind3_dictated_appropriation_amount: cardinal;
...
begin
  ...
  if IsPostback and (session['p'].GetType.namespace = p.GetType.namespace) then begin
    ...
    be_stale_password := Borland.Data.Provider.BdpCommand.Create
      (
      'SELECT be_stale_password FROM kind1_user where id=' + session['kind1_user_id'].tostring,
      p.db.connection
      )
    ...
    p.tcci_kind3_name := 2;
    p.tcci_kind3_dictated_appropriation_id := 3;
    p.tcci_kind3_dictated_appropriation_amount := 4;
    ...
    biz_get_profile_status := borland.data.provider.bdpCommand.Create
      (
      'select be_valid_profile from kind1 where id = "' + session['kind1_user_id'].ToString + '"'
      ,p.db.connection
      );
...
//        +   ' and name = "emsof-kind1-item-requests-deadline"',
//        p.db.connection
//        )
//        .ExecuteScalar
//      );
...
procedure TWebForm_kind1_overview.DataGrid_ItemCommand(source: System.Object;
...
begin
  ...
  session.Remove('sponsor_kind3');
  session.Add('sponsor_kind3',Safe(e.item.cells[p.tcci_kind3_name].text,POSTAL_CITY));
  session.Remove('kind3_dictated_appropriation_id');
  session.Add('kind3_dictated_appropriation_id',Safe(e.item.cells[p.tcci_kind3_dictated_appropriation_id].text,REAL_NUM));
  session.Remove('kind3_dictated_appropriation_amount');
  session.Add('kind3_dictated_appropriation_amount',Safe(e.item.cells[p.tcci_kind3_dictated_appropriation_amount].text,REAL_NUM));
  server.Transfer('request_overview.aspx');
end;

procedure TWebForm_kind1_overview.DataGrid_ItemDataBound(sender: System.Object;
...
procedure TWebForm_kind1_overview.BindDataGrid;
var
...
begin
  ...
  DataGrid.DataSource := borland.data.provider.bdpcommand.Create
    ...
    + ' name as kind3_name,'                                                          // column 2
    + ' kind3_dictated_appropriation.id as kind3_dictated_appropriation_id,'         // column 3
    + ' kind3_dictated_appropriation.amount as kind3_dictated_appropriation_amount,' // column 4
    ...
    +   ' JOIN kind3_dictated_appropriation'
    +     ' on (kind3_dictated_appropriation.id=emsof_request_master.kind3_dictated_appropriation_id)'
    +   ' JOIN region_dictated_appropriation'
    +     ' on (region_dictated_appropriation.id=kind3_dictated_appropriation.region_dictated_appropriation_id)'
    ...
    +   ' JOIN kind3_code_name_map on (kind3_code_name_map.code=region_dictated_appropriation.kind3_code)'
    +   ' JOIN request_status_code_description_map on (emsof_request_master.status_code = request_status_code_description_map.code)'
    + ' WHERE kind1_id = ' + session['kind1_user_id'].tostring
    +   ' and fiscal_year.id >= (' + p.max_fiscal_year_id_string + ' - 1)'
    + ' order by fy_designator,kind3_name',
    p.db.connection
    )
    .ExecuteReader;
  DataGrid.DataBind;
