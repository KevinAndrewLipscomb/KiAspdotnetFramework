      ...
      public string sort_order;
            ...
            if (row.Cells.Count > Static.CI_FIRST_CROSSTAB)
            {
                for (i = Static.CI_FIRST_CROSSTAB; i <= (row.Cells.Count - 1); i ++ )
                {
                    if (row.RowType == DataControlRowType.DataRow)
                    {
                        ...
                        check_box.ID = k.EMPTY + Static.CHECKBOX_ID_PREFIX_MEMBER_ID + row.Cells[Static.CI_MEMBER_ID].Text + Static.CHECKBOX_ID_PREFIX_ROLE_ID + crosstab_metadata_rec.id;
        ...
        private void GridView_control_RowDataBound(object sender, System.Web.UI.WebControls.GridViewRowEventArgs e)
        {
            if (e.Row.RowType != DataControlRowType.EmptyDataRow)
            {
                ...
                e.Row.Cells[Static.CI_MEMBER_NAME].Wrap = false;
                Checkboxify(e.Row);
            }
