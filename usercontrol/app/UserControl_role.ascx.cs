...
namespace UserControl_role
  ...
  public partial class TWebUserControl_role : ki_web_ui.usercontrol_class
    ...
    private bool PresentRecord(string name)
      ...
      result = false;
      if (p.biz.roles.Get(name, out soft_hyphenation_text, out pecking_order))
        {
    ...
    protected void Button_send_Click(object sender, System.EventArgs e)
      ...
      k.SmtpMailSend
        ...
        message_string:$"-- From {p.biz.user.Roles()[0]} {p.biz.members.FirstNameOfMemberId(Session["member_id"].ToString())} {p.biz.members.LastNameOfMemberId(Session["member_id"].ToString())} ({p.biz.user.EmailAddress()}) [via {ConfigurationManager.AppSettings["application_name"]}]" + k.NEW_LINE
        + k.NEW_LINE
        + TextBox_quick_message_body.Text,
        be_html:false,
        cc:k.EMPTY,
        bcc:p.biz.user.EmailAddress(),
        reply_to:p.biz.user.EmailAddress()
        );
    ...
    protected void Button_submit_Click(object sender, System.EventArgs e)
      ...
      if (Page.IsValid)
        {
        p.biz.roles.Set(k.Safe(TextBox_name.Text, k.safe_hint_type.HUMAN_NAME).Trim(), k.Safe(TextBox_soft_hyphenation_text.Text, k.safe_hint_type.PUNCTUATED).Trim(), k.Safe(TextBox_pecking_order.Text, k.safe_hint_type.NUM).Trim());
    ...
    protected void Button_lookup_Click(object sender, System.EventArgs e)
      ...
      if (!PresentRecord(saved_name))
        ...
        p.biz.roles.Bind(saved_name, DropDownList_name);
    ...
    private void BindHolders(string role_name)
      ...
      p.biz.role_member_map.BindHolders(role_name, GridView_holders, p.sort_order, p.be_sort_order_ascending);
      p.be_gridview_empty = (p.num_gridview_rows == 0);
      Table_holders.Visible = !p.be_gridview_empty;
      if (v.distribution_list.Length > 0)
        v.distribution_list.Remove(0, 2);
